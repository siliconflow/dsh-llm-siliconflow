...
const noopResolveImage = (): Promise<string | undefined> => Promise.resolve(undefined)
...
function replaceImagesWithSentinel(blocks: readonly ContentBlock[]): ContentBlock[] {
  return blocks.map(block =>
    block.type === 'image' ? { type: 'text', text: OFFLOADED_IMAGE_TEXT } : block,
  )
}
    ...
    if (block.type === 'text') {
      ...
      parts.push(
        url !== undefined
          ...
          : { type: 'text' as const, text: OFFLOADED_IMAGE_TEXT },
      )
    }
...
export async function serializeMessages(
  messages: Message[],
  resolveImage: (ref: ImageAttachmentRef) => Promise<string | undefined> = noopResolveImage,
): Promise<WireMessage[]> {
