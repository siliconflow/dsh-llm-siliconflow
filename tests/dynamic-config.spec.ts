...
async function home(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'dsh-llm-siliconflow-dynamic-'))
 ...
 * llm-siliconflow over one temp harness home. `watch: false` keeps every change
...
async function boot(dir: string, config: object): Promise<Harness> {
  ...
  await ctx.plugin(LlmSiliconFlow, config)
  return { ctx, settingsFiber }
}

function prompt(ctx: Context) {
  return assemble(ctx, { model: MODEL, messages: [] })
}
...
describe('request-level dynamic configuration', () => {
  ...
  it('interrogates a stored route using the stored credential', async () => {
    ...
    const models = await ctx.llm.discoverModels(NS, { provider: 'siliconflow' })
    expect(models).toEqual([{ id: 'stored' }])
  ...
  it('re-registers the route in place when the captured retry policy changes, without an empty-registry window', async () => {
    ...
    expect(observed).toEqual([['siliconflow']])
  })
  ...
  it('keeps the whole last-good snapshot when a rejected one changed the URL', async () => {
    ...
    vi.stubEnv('SILICONFLOW_API_KEY', 'good-key')
