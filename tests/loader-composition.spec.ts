 ...
 * settings-file, credentials-local, and llm-siliconflow boot from a test-only
...
async function loadComposition(
  ...
  root = options.reuseRoot ?? await mkdtemp(join(tmpdir(), 'dsh-llm-siliconflow-composition-'))
...
describe('llm-siliconflow real dynamic composition', () => {
  it('boots from cordis.yml and routes the next request after external settings and credential edits', async () => {
    ...
    expect(serverA.headers[0]?.['x-siliconflow-harness-user-id']).toBe(getOrCreateAnonymousUserId())
  ...
  it('keeps a stored key writable and rotatable across a real restart', async () => {
    ...
    await assemble(boot.ctx, { model: MODEL, messages: [] })
    ...
    await assemble(restarted.ctx, { model: MODEL, messages: [] })
  ...
  it('boots the same adapter on entry config alone, resolving the reference from the environment', async () => {
    vi.stubEnv('SILICONFLOW_API_KEY', 'entry-key')
