...
`@deepseek-ai/dsh-*` 依赖尚未发布，因此本包在固定的 DeepSeek Harness 检出内构建与测试——CI 工作流把它检出到 `packages/llm/llm-siliconflow` 并运行 harness 的检查门。待这些依赖发布后，用 `dsh plugin --profile <name> add @siliconflow/dsh-llm-siliconflow` 装进 profile，或直接在 `cordis.patch.yml` 层挂载：
